...
void print_system_time(FILE *out)
{
	SYSTEMTIME systime;
...
void bills_details()
{
	...
	while (1)
	{
		...
		if (pos==-1) continue;
		++pos;
		for (int i=0; i+pos<=strlen(st); ++i) st[i]=st[i+pos];
		double num=strtod(st, &fail);
		total+=num;
	}
	fclose(secret);
	fclose(bill);
...
int main()
{
	...
	while (1)
	{
		...
		printf("1.Add bills.    2.Check the bills.    3.Change KEY.    4.Bills details.    5.Exit\n");
		int nid=getnum(5);
		bool done=false;
		switch (nid)
		{
